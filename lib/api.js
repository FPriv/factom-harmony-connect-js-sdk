...
export default class APICall {
  ...
  */
  async send(method, url = '', data = {}, overrideBaseUrl = '', overrideAccessToken = null) {
    const baseUrl = overrideBaseUrl || this.baseUrl;
    if (!isUrl(baseUrl)) throw new Error('The base URL provided for override is not valid.');
    let callURL = urlJoin(baseUrl, url);
    const headers = createAuthHeader(overrideAccessToken || this.accessToken);
    let body = '';
