...
export default class IdentitiesKeyUtil {
   ...
   * @param  {String} params.signerPublicKey
  ...
  async get(params = {}) {
    ...
    if (CommonUtil.isEmptyString(params.signerPublicKey)) {
      throw new Error('signerPublicKey is required.');
    }
    if (!KeyCommon.validateCheckSum({ signerKey: params.signerPublicKey })) {
      throw new Error('signerPublicKey is invalid.');
    }

    const response = await this.apiCall.send(
      constants.GET_METHOD,
      `${constants.IDENTITIES_URL}/${params.identityChainId}/${constants.KEYS_STRING}/${params.signerPublicKey}`,
    );
  ...
  async replace({ identityChainId = '', ...params } = {}) {
    ...
    if (!KeyCommon.validateCheckSum({ signerKey: params.newPublicKey })) {
