...
export default class KeyUtil {
  ...
  /**
   * @param  {String} params.privateKey='' - A base58 string in idpub or idsec format
   * @return  {String} Returns a base58 string in idpub format
   */
  static getPublicKeyFromPrivateKey({ privateKey = '' } = {}) {
    if (!this.validateCheckSum({ signerKey: privateKey })) {
      throw new Error('private key is invalid.');
    }

    const privateKeyBytes = this.getKeyBytesFromKey({ signerKey: privateKey });
    const publicKeyBytes = ec.keyFromSecret(privateKeyBytes).getPublic();
  ...
  static signContent({ privateKey = '', message = '' } = {}) {
    ...
    if (!KeyUtil.validateCheckSum({ signerKey: privateKey })) {
      throw new Error('private key is invalid.');
    }
  ...
  /**
   * @param  {String} params.publicKey='' - A base58 string in idpub format
  ...
  static validatingSignature({ publicKey = '', signature = '', message = '' } = {}) {
    ...
    if (!KeyUtil.validateCheckSum({ signerKey: publicKey })) {
    ...
    const msgBytes = Buffer.from(message, utf8Encode);
    const keyBytes = this.getKeyBytesFromKey({ signerKey: publicKey });
