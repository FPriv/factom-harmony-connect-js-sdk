...
export default class ValidateSignatureUtil {
  ...
  static async validateSignature({
    ...
    try {
      const keyResponse = await apiCall.send(constants.GET_METHOD,
        `${constants.IDENTITIES_URL}/${signerChainId}/${constants.KEYS_STRING}/${signerPublicKey}`,
        {}, clientOverrides);
