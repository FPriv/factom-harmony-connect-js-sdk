...
export default class Identity {
  ...
  async createAnIdentity(params = {}) {
    ...
    nameBase64.forEach((o) => {
      nameByteCounts += Buffer.from(o).length;
    });
    const totalBytes = 37 + nameByteCounts + (2 * nameBase64.length) + (params.keys.length * 58);
    if (totalBytes > 10240) {
   ...
   */
  async getAnIdentity({ identityChainId = '' } = {}) {
    if (!identityChainId) {
      throw new Error('identity chain id is required.');
    }
    const response = await this.apiCall.send(getMethod, `${identitiesUrl}/${identityChainId}`);
  ...
  async getAllIdentityKeys({ identityChainId = '', ...params } = {}) {
    ...
    const response = await this.apiCall.send(getMethod, `${identitiesUrl}/${identityChainId}/${keysString}`, data);
  ...
  /**
   * @param  {String} params.identityChainId='' The chain id of the desired Identity Keys.
   * @param  {String} params.oldSignerPublicKey=''
   * @param  {String} params.newSignerPublicKey=''
   ...
   */
  async createAnIdentityKeyReplacement({ identityChainId = '', ...params } = {}) {
    if (CommonUtil.isEmptyString(identityChainId)) {
      throw new Error('identity chain id is required.');
    }
    if (CommonUtil.isEmptyString(params.oldSignerPublicKey)) {
      throw new Error('old public key is required.');
    }
    if (CommonUtil.isEmptyString(params.newSignerPublicKey)) {
      throw new Error('new public key is required.');
    }
    if (CommonUtil.isEmptyString(params.signerPrivateKey)) {
      throw new Error('private key is required.');
    }
    if (!KeyUtil.validateCheckSum({ signerKey: params.oldSignerPublicKey })) {
      throw new Error('old public key is invalid.');
    }
    if (!KeyUtil.validateCheckSum({ signerKey: params.newSignerPublicKey })) {
    ...
    const msgHash = Buffer.from(`${identityChainId + params.oldSignerPublicKey + params.newSignerPublicKey}`, utf8Encode);
    ...
    const data = {
      old_key: params.oldSignerPublicKey,
      new_key: params.newSignerPublicKey,
      signature: signature,
    ...
    const response = await this.apiCall.send(postMethod, `${identitiesUrl}/${identityChainId}/${keysString}`, data);
